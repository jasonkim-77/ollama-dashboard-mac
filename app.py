...
with st.expander("🖥️ 1. 시스템 상태 대시보드", expanded=True):
    ...
    # OS가 macOS인지 판별 (이전 get_system_metrics 리턴값 구조에 맞춰 대응)
    is_osx = sys_metrics.get("os") == "macOS" or sys_metrics.get("is_osx", False)

    # OS가 맥이면 레이아웃을 2열(CPU, RAM)로, 리눅스/윈도우면 3열(VRAM 포함)로 유연하게 구성
    if is_osx:
    ...
    with sys_col2:
        st.markdown(f"**시스템 메모리 (RAM / Unified Memory)**")
...
if not os.path.exists(LOG_PATH):
    ...
    # ==========================================
    with st.expander("🔮 2. 하드웨어 기반 컨텍스트 확장 한계 예측", expanded=True):
    
        if is_osx:
            # 🍏 macOS: 통합 메모리(Unified Memory) 기준 예측 로직 적용
            # 전체 시스템 RAM 잔여 용량 기반 연산
            free_ram = sys_metrics["ram_total"] - sys_metrics["ram_used"]
            ram_for_extension = free_ram - 4.0  # macOS 시스템 안정성을 위해 4GB 마진 공제
            kv_cache_factor_per_1k = 0.45 
            
            max_additional_tokens_k = max(0.0, ram_for_extension / kv_cache_factor_per_1k)
            ...
            with advice_col3:
                st.success(f"🍏 **통합 가용 메모리 여유**: `{free_ram:.2f} GB` (안전 마진 4GB 기준)")
                
            if ram_for_extension > 0.1:
                st.markdown(f"""
                <div class="advice-box" style="border-left: 5px solid #007aff;">
                    🍏 <b>macOS 통합 메모리 컨텍스트 가이드:</b><br>
                    Apple Silicon은 CPU와 GPU가 <b>통합 메모리(Unified Memory)</b>를 공유합니다.<br>
                    시스템 잔여 메모리를 고려할 때, Modelfile의 <code>num_ctx</code>를 최대 <b>{recommended_max_ctx:,} tokens</b> 수준까지 확장해볼 수 있습니다.<br>
                    <i>*주의: 컨텍스트 확장 시 브라우저나 타 앱의 RAM 점유율 상황에 따라 Swap 메모리가 발생할 수 있습니다.</i>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown(f"""
                <div class="advice-box" style="background-color: #2c2c2e; border-left: 5px solid #ff9500; color: white;">
                    ⚠️ <b>통합 메모리 압박 상태:</b><br>
                    현재 macOS 시스템 여유 RAM이 무척 타이트합니다.<br>
                    컨텍스트 사이즈를 늘리면 메모리 압박(Memory Pressure)이 증가하여 시스템 전체 멈춤이나 대형 스와프(Swap)가 유발될 수 있으므로 현재 설정을 유지하는 것을 권장합니다.
                </div>
                """, unsafe_allow_html=True)
                
        else:
            # 🐧 리눅스 / 외장 GPU 장착 일반 PC: 기존 VRAM 전용 로직 작동
